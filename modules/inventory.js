...
function rnInventory(){
  ...
  list.forEach(function(item){
    ...
    html += '<div style="background:var(--surface);border:1.5px solid var(--b1);border-radius:var(--radius);padding:16px;position:relative;overflow:hidden;transition:all .18s" onmouseenter="this.style.borderColor=\''+statusColor+'\';this.style.boxShadow=\'0 6px 20px rgba(0,0,0,.08)\'" onmouseleave="this.style.borderColor=\'var(--b1)\';this.style.boxShadow=\'none\'">'
      ...
      +'<div style="font-size:11px;color:var(--muted);margin-top:1px">'+esc(item.category)+' · 單位：'+esc(item.unit||'個')+'</div></div>'
      +'<span style="font-size:10px;padding:2px 8px;border-radius:99px;font-weight:800;background:'+statusBg+';color:'+statusColor+';white-space:nowrap;margin-left:6px">'+statusLabel+'</span>'
      +'</div>'
      ...
      +'<div style="font-size:10px;color:var(--faint);margin-bottom:8px">最低庫存：'+item.minQty+' '+esc(item.unit||'個')+'</div>'
    ...
    html += '<div class="sec-label">最近異動紀錄</div>'
      ...
      +logs.map(function(l){
        ...
        return '<tr>'
          ...
          +'<td><span style="font-size:10px;padding:2px 8px;border-radius:99px;font-weight:700;background:'+(isUse?'var(--red-bg)':'var(--green-bg)')+';color:'+(isUse?'var(--red)':'var(--green)')+'">'+( isUse?'使用':'補貨')+'</span></td>'
