...
function openAddDept(){showModal('新增科別',`<div class="form-row"><label>科別名稱</label><input id="deptName" placeholder="例：內科部"></div>`,()=>{const n=document.getElementById('deptName').value.trim();if(!n)return;store.departments.push({id:uid(),name:n});logAudit('新增科別', n);saveCollection('departments');closeModal();renderDeptContent();});}
function openEditDept(id){const d=store.departments.find(x=>x.id===id);showModal('編輯科別',`<div class="form-row"><label>科別名稱</label><input id="deptName" value="${esc(d.name)}"></div>`,()=>{const n=document.getElementById('deptName').value.trim();if(!n)return;d.name=n;logAudit('編輯科別', n);saveCollection('departments');closeModal();renderDeptContent();});}
function deleteDept(id){const m=store.users.filter(u=>u.deptId===id);if(m.length&&!confirm(`此科別有 ${m.length} 位成員，確定刪除？`))return;const dd=store.departments.find(x=>x.id===id);store.departments=store.departments.filter(x=>x.id!==id);store.users.forEach(u=>{if(u.deptId===id)u.deptId='';});logAudit('刪除科別', dd?dd.name:'');saveMultiple(['departments','users']);renderDeptContent();}
