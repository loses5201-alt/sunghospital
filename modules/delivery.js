...
function rnRooms(){
  ...
  const cards=store.rooms.map((r,i)=>{const s=RSTS[r.status]||RSTS.empty;return'<div class="rcard '+s.rc+'" onclick="editRoom('+i+')"><span style="font-size:26px;margin-bottom:7px;display:block">'+s.i+'</span><div style="font-size:13px;font-weight:700">'+esc(r.name)+'</div><span style="font-size:12px;padding:2px 8px;border-radius:99px;font-weight:600;margin-top:5px;display:inline-block;background:'+(r.status==='empty'?'#e8f7f0':r.status==='waiting'?'#fdf0dc':r.status==='active'?'#fce8e8':r.status==='recovery'?'#e8f0fb':'#f0f0f0')+';color:'+(r.status==='empty'?'#2e7d5a':r.status==='waiting'?'#8f5208':r.status==='active'?'#b03050':r.status==='recovery'?'#1558a0':'#888')+'">'+s.l+'</span>'+(r.patient?'<div style="font-size:12px;color:var(--muted);margin-top:3px">'+esc(r.patient)+'</div>':'')+(r.since?'<div style="font-size:11px;color:var(--faint);margin-top:2px">⏱ '+esc(r.since)+'</div>':'')+(r.note?'<div style="font-size:11px;color:var(--faint);margin-top:2px">'+esc(r.note)+'</div>':'')+'</div>';}).join('');
