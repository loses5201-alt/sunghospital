...
function renderJournalPage(c){
  c.innerHTML='<div class="admin-layout" style="height:100%;display:flex;flex-direction:column">'
    ...
    +'<input class="board-search" id="boardSearch" placeholder="🔍 搜尋貼文…" oninput="_boardSearch=this.value;rnBoard()" value="'+esc(_boardSearch)+'">'
    +'<div class="board-cats">'
    +'<button class="bc-filter-btn'+(  _boardFilter==='all'?' active':'')+'" onclick="_boardFilter=\'all\';rnBoard()">全部</button>'
    +Object.entries(BOARD_CATS).map(function(kv){var k=kv[0],v=kv[1];return'<button class="bc-filter-btn'+(  _boardFilter===k?' active':'')+'" onclick="_boardFilter=\''+k+'\';rnBoard()">'+v.e+' '+v.l+'</button>';}).join('')
    +'</div>'
